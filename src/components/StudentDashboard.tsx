...
export const StudentDashboard: React.FC<StudentDashboardProps> = ({
  ...
  const getSubjectIcon = (iconName: string) => {
    switch (iconName) {
      ...
      case 'TrendingUp': return <BarChart3 className="w-5 h-5" />;
      case 'Globe':
      case 'Landmark': return <Globe className="w-5 h-5" />;
      ...
      <div className="bg-white rounded-3xl p-5 sm:p-6 border border-slate-200/90 shadow-sm flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-2 self-end sm:self-auto">
          ...
          <button
            ...
            className="px-2.5 py-2 text-[11px] font-bold rounded-2xl bg-slate-100 hover:bg-slate-200 text-slate-700 transition-colors cursor-pointer"
          ...
          {onLogout && (
            <button
              type="button"
              onClick={onLogout}
              className="px-2.5 py-2 text-[11px] font-bold rounded-2xl bg-rose-50 hover:bg-rose-100 text-rose-700 border border-rose-200 transition-colors flex items-center gap-1 cursor-pointer"
